 ...
 */
const string kYourName = "Max Chang";
...
void moveToNextNode(MazeCell** nodeptr, char move) {
    switch (move)
    {
        case 'N':
            *nodeptr = (*nodeptr)->north;
            break;
        case 'S':
            *nodeptr = (*nodeptr)->south;
            break;
        case 'E':
            *nodeptr = (*nodeptr)->east;
            break;
        case 'W':
            *nodeptr = (*nodeptr)->west;
            break;
         default:
            error("Invalid Move " + string(1, move));
    }

}

void pickItem(MazeCell* node, Set<string>& picked) {
    if (node->contents != "") picked.add(node->contents);
}

bool allItemsPicked(Set<string>& picked, Set<string>& needs) {
    return needs.isSubsetOf(picked);
}
...
bool isPathToFreedom(MazeCell* start, string moves, Set<string> needs) {
    ...
    // if item is at start location
    pickItem(node, picked);
    if (allItemsPicked(picked, needs)) return true;

    // moves iteratively
    for (char move: moves) {
        moveToNextNode(&node, move);
