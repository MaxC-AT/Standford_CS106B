...
long smarterSum(long n) {
    ...
    long searchLimit = sqrt(n);
    int isSqrtNum = (searchLimit * searchLimit == n)? 1 : 0;

    long total = 1;
    for (long divisor = 2; divisor <= searchLimit - isSqrtNum; divisor++) {
        if (n % divisor == 0) {
            total += divisor + n / divisor;
        }
    }
    if (isSqrtNum) total += searchLimit;
    return total;
}
...
long findNthPerfectEuclid(long n) {
    ...
    long cnt = 0;
    while (true) {
        if (smarterSum(pow(2, k) - 1) == 1) cnt += 1;
        if (cnt == n) break;
        k += 1;
    }
...
STUDENT_TEST("Confirm findNthPerfectEuclid of first 5 perfect number") {
